...
import { ServiceSale, SearchResult } from '../../reports/reports.model';
...
export class AgentSalesReportsService {
   ...
   * Search Method
   */
  private _search(salesServices): Observable<SearchResult> {
    const {
      sortColumn,
      sortDirection,
      pageSize,
    ...
    // 1. sort
    let rows = sort(salesServices, sortColumn, sortDirection);
  ...
  public getSumOfTransactions(
    serviceId: number,
    msisdn: string
  ): Observable<ServiceSale[]> {
    // this.transactionsService._fetchTransactionsByAgent(msisdn);
    let url = `${this.reportsUrl}/records/agent-transactions-by-serviceId/${msisdn}/${serviceId}`;
  ...
  public _fetchSales(msisdn: string) {
    this._services.forEach(({ serviceId, serviceName }) => {
      this.getSumOfTransactions(serviceId, msisdn).subscribe(
        (transactionSum) => {
  ...
  public exportSales(fileName: string, dateFrom?: Date, dateTo?: Date) {
    ...
    // Export data
    let tx: Transaction[] = this.transactionsService.agentTransactionData;
    let salesExport;
